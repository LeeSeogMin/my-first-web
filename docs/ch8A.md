...
```text
너는 GitHub Copilot Chat이야. 지금부터 `my-first-web` 프로젝트의 Supabase를 "처음부터" 세팅한다.
...
목표: `my-first-web`에 Supabase를 연결하고, `ARCHITECTURE.md`/`supabase-schema.sql` 기준으로 테이블을 만들고, Next.js에서 읽기 테스트까지 끝낸다.
...
Ch6까지 만든 블로그는 `lib/posts.ts`의 더미 데이터를 사용한다. 새로고침해도 데이터는 보이지만 **새 글을 추가하거나 수정할 수 없다** — 하드코딩된 배열이기 때문이다. 이번 장에서는 이 더미 데이터를 **Supabase의 실제 데이터베이스**로 교체한다. 같은 `/posts` 경로, 같은 UI, 같은 컴포넌트 — **데이터 소스만 바뀐다**. 이것이 프론트엔드(Ch1~6)에서 백엔드(Ch8+)로 넘어가는 핵심 전환점이다.
...
Supabase JS 클라이언트는 버전마다 API가 자주 바뀐다. 코드 작성 전에 Context7로 최신 사용법을 먼저 확인한다.

**언제 쓰는가**: `@supabase/ssr` 설치/설정 전, 클라이언트 초기화 코드 작성 전

```
use context7. @supabase/ssr Next.js App Router에서 서버 컴포넌트용 클라이언트와
브라우저용 클라이언트를 분리하는 현재 권장 방법을 알려줘.
```

```
use context7. @supabase/ssr createServerClient와 createBrowserClient의
현재 올바른 사용법과 쿠키 설정 방법을 알려줘.
```

> **주의**: Copilot이 구버전 `createClient` 단일 방식으로 만들 수 있다. Context7로 확인 후 서버/브라우저 분리 방식인지 반드시 점검한다.
