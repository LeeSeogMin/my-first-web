...
1. [https://nodejs.org](https://nodejs.org/) 에 접속한다
...
1. [https://code.visualstudio.com](https://code.visualstudio.com/) 에 접속한다
...
- **Windows**: [https://git-scm.com](https://git-scm.com/) 에서 다운로드하여 설치한다. 설치 옵션은 기본값을 유지한다.
- **macOS**: 터미널에서 `git --version`을 입력하면 자동 설치 안내가 나온다. 또는 [https://git-scm.com](https://git-scm.com/) 에서 다운로드한다.
...
1. [https://github.com](https://github.com/) 에 접속하여 회원가입한다
...
| `pwd` (macOS) / `cd` (Windows, 인수 없이) | 현재 위치 확인      | `pwd`            |
...
```bash
C:\Users\user>cd \     <- 원화표시 키보드가 \ 임
C:\npx create-next-app@latest my-first-web
```

설치 중 질문이 나오면 다음과 같이 선택한다:

```
C:\>npx create-next-app@latest my-first-web
...
```
next: 16.2.1    ← 실행 시점에 따라 다르다 (16.x, 17.x 등이 될 수 있다)
```
...
이 상태에서 `app/page.tsx`를 수정하면 브라우저에 즉시 반영된다. 이것을 **핫 리로드**(Hot Reload)라 한다.
...
1. GitHub([https://github.com](https://github.com/))에 로그인한다
...
1. [https://vercel.com](https://vercel.com/) 에 접속한다
...
4. Congratulations! -> Continue to Dashboard 클릭
5. 대시보드 화면에서 Domain : my-first-web-phi-two.vercel.app 클릭 <- 나의 공식 홈페이지 주소
...
1. `app/page.tsx` 파일에서 기본 Next.js 템플릿 내용을 삭제한다
