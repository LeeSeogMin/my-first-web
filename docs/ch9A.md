...
|  ②   | AuthContext + useAuth 전역 상태 · Navbar.tsx UI  |  🤖  |  9.4  |
...
```tsx
// components/Navbar.tsx (인증 관련 부분)
"use client";
...
- 전역 상태: AuthContext + useAuth Hook (contexts/AuthContext.tsx)
...
3. `contexts/AuthContext.tsx` — AuthProvider + useAuth
4. `components/Navbar.tsx` — 로그인/로그아웃 UI (기존 헤더에 통합)
...
- 권장 타이밍: `auth.ts` + `AuthContext.tsx` 작성 후 1회.

### Context7 활용 가이드

Supabase Auth API는 버전마다 메서드명이 바뀐다 (`signIn` → `signInWithPassword` 등). 코드 작성 전에 반드시 확인한다.

**언제 쓰는가**: 로그인/로그아웃/회원가입 코드 작성 전, 미들웨어 설정 전

```
use context7. @supabase/ssr 현재 버전에서 이메일/비밀번호 로그인(signInWithPassword),
로그아웃(signOut)의 올바른 사용법을 알려줘. Next.js App Router 기준.
```

```
use context7. @supabase/ssr Next.js App Router에서 인증 상태를 미들웨어로
보호하는 현재 권장 방법(middleware.ts)을 알려줘.
```

> **주의**: Copilot이 `signIn` (구버전)으로 만들면 런타임 에러가 난다. Context7로 확인 후 `signInWithPassword`인지 점검한다.

B회차에서는 Ch8에서 만든 블로그 프로젝트를 이어서 사용한다.
