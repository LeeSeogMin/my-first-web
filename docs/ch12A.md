...
[요구 출력]
1) 에러/로딩/빈 상태를 어디에 넣을지: `error.js`, `loading.js`, 컴포넌트 레벨 중 추천
...
```mermaid
graph LR
  A["① error.js<br/>에러 처리"] --> B["② loading.js<br/>+ 스켈레톤 UI"]
  B --> C["③ 폼 유효성<br/>검증"]
...
| 단계 | 내용                          | 실행 |   절   |
| :--: | ----------------------------- | :--: | :----: |
|  ①   | error.js 에러 처리 페이지    |  🤖  | 12.2.2 |
|  ②   | loading.js + 스켈레톤 UI     |  🤖  |  12.3  |
|  ③   | 폼 유효성 검증 (클라이언트)   |  🤖  |  12.4  |
...
```typescript
// 에러 처리 유틸리티 함수
export async function fetchPosts(): Promise<{ data: Post[] | null; error: string | null }> {
  try {
...
// 에러 코드 → 사용자 메시지 변환
function getUserMessage(error: { code?: string; message?: string }): string {
...
### 12.2.2 Next.js error.js 활용 `🤖 바이브코딩`
...
> **라이브 코딩**: error.js를 함께 만들고, 의도적으로 에러를 발생시켜 동작을 확인한다
...
### 12.3.1 Next.js loading.js 활용 `🤖 바이브코딩`
...
```tsx
// components/PostForm.tsx (유효성 검증 추가 버전)
"use client";
...
export default function PostForm({ onSubmit }: { onSubmit: (data: { title: string; content: string }) => void }) {
...
페이지별로 다른 메타데이터도 설정할 수 있다. 이 교재 고정 버전(Next.js 16.2.1)에서는 `params`를 Promise로 받는다:
...
에러 처리와 UX 개선은 **여러 파일에 분산**되어 있다(error.js, loading.js, 각 폼 컴포넌트 등). context.md에 "어떤 페이지에 어떤 UX 처리를 적용했는지" 기록하면, 새 페이지를 만들 때 Copilot이 동일한 패턴을 적용한다.
...
기존 페이지에 error.js, loading.js를 추가하고 폼 유효성 검증을 넣을 거야.
```
...
- 에러 처리: Next.js error.js (폴더별 자동 적용)
- 로딩 상태: Next.js loading.js + Tailwind 스켈레톤 UI
...
- error.js의 reset() 호출 후에도 에러 반복 → 원인이 되는 데이터 문제를 먼저 해결해야 함
- 스켈레톤 UI 깜빡임 → Suspense boundary 위치 조정

## 알게 된 점 (Ch12)

- error.js는 반드시 "use client" 컴포넌트여야 한다
...
- 알게 된 점: error.js/loading.js 주의사항
...
2. `app/loading.js` — 로딩 스피너 또는 스켈레톤 UI (components/PostListSkeleton.js)
...
- `nextjs-basic-check`: `error.js`, `loading.js` 파일 위치와 App Router 적용 범위를 점검한다.
- 권장 타이밍: UX 개선 완료 후 제출 직전.

B회차에서는 Ch11에서 만든 블로그 프로젝트를 이어서 사용한다.
