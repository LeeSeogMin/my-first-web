...
5. **산출물을 문서로 고정**: `ARCHITECTURE.md`, `copilot-instructions.md` 같은 “고정된 맥락”을 만들어 다음 장의 Copilot 프롬프트 품질을 올린다.
...
1. **문서부터**: `ARCHITECTURE.md`에 목표/페이지맵/유저플로우/디자인토큰을 먼저 적어 “정답지(맥락)”를 만든다.
2. **규칙부터**: `copilot-instructions.md`에 코딩 규칙(폴더 규칙, 컴포넌트 규칙, 토큰)을 적어 “추측 금지”를 만든다.
...
```text
너는 GitHub Copilot Chat이야. 지금부터 `my-blog`(개인 블로그) 프로젝트의 아키텍처를 보강한다.
...
주의: Ch1~6에서 기본 코드가 있으니, 기존 파일 구조를 존중하면서 규칙을 작성해줘.
```

#### (3) 구현 시작: App Router 뼈대/네비게이션

```text
너는 GitHub Copilot Chat이야. 이제 문서(ARCHITECTURE.md, copilot-instructions.md)를 기준으로 기존 블로그의 구조를 보강한다.
...
```text
너는 GitHub Copilot Chat이고, 내 `my-blog`(개인 블로그) 프로젝트의 제품/프론트엔드 설계 파트너야.
...
3. Copilot Vision으로 스케치를 코드로 변환하고, v0를 활용하여 프로토타입을 생성할 수 있다
4. shadcn/ui를 설치하고 디자인 토큰으로 테마를 커스터마이징할 수 있다
5. 설계서(ARCHITECTURE.md + copilot-instructions.md)를 작성하여 Copilot에게 프로젝트 컨텍스트를 제공할 수 있다
...
### 7.6.4 설계 문서를 참조하여 AI에게 코드 생성 지시하기
...
### 7.6.5 Copilot의 기억 한계: 왜 외부 컨텍스트 파일이 필요한가
...
`copilot-instructions.md`는 **규칙**(항상 자동 로드)이고, `ARCHITECTURE.md`는 **설계도**이다. 하지만 **"지금 어디까지 했는지"**(상태)와 **"다음에 뭘 해야 하는지"**(할 일)는 어디에도 없다.

이 문제를 해결하는 것이 **외부 컨텍스트 파일**이다:

**표 7.11** 설계 문서 4종 세트 — AI 컨텍스트의 완전한 구조

| 파일                      | 역할                       | Copilot 자동 로드 | 비유       |
| ------------------------- | -------------------------- | :---------------: | ---------- |
| `copilot-instructions.md` | 규칙 — "이렇게 코딩해라"   |        ✅         | 회사 사규  |
| `ARCHITECTURE.md`         | 설계 — "뭘 만들고 있다"    |        ❌         | 건축 도면  |
| `context.md`              | 상태 — "여기까지 했다"     |        ❌         | 작업 일지  |
| `todo.md`                 | 할 일 — "다음에 이걸 한다" |        ❌         | 체크리스트 |

> **핵심**: `copilot-instructions.md`만 자동 로드된다. 나머지 3개는 Copilot Chat에서 **`#file:파일명`으로 직접 참조**해야 한다.

### 7.6.6 context.md — 프로젝트 상태 기록
...
- Tailwind CSS 3.4.17 기준에서는 `tailwind.config.js` + `globals.css` 조합으로 설정
...
**업데이트 시점**: 작업 세션을 마칠 때마다 context.md를 갱신한다. Copilot에게 요청할 수 있다:
...
**업데이트 시점**: 작업을 시작하기 전에 todo.md를 확인하고, 완료된 항목은 체크(`[x]`)한다. Copilot에게 요청할 수 있다:

```text
방금 포스트 상세 페이지를 완성했어. todo.md에서 해당 항목을 체크하고 진행률을 업데이트해줘.
```

### 7.6.8 세션 시작 프롬프트: Copilot에 외부 맥락 주입하기

외부 컨텍스트 파일의 진짜 힘은 **Copilot Chat을 새로 열 때 한꺼번에 주입**할 때 나온다.

새 대화를 열고 가장 먼저 입력하는 **세션 시작 프롬프트**:
...
copilot-instructions.md의 규칙을 지켜서 작업해줘.
```

> **팁**: `copilot-instructions.md`는 `#file:`로 참조하지 않아도 **자동 로드**된다. 나머지 3개만 명시하면 된다.

이 프롬프트 하나로 Copilot은:

1. **어디까지 했는지** 파악하고 (context.md)
2. **다음에 뭘 할지** 알고 (todo.md)
3. **어떤 구조로 만들지** 이해하고 (ARCHITECTURE.md)
4. **어떤 규칙으로 코딩할지** 준수한다 (copilot-instructions.md — 자동)

```
세션 시작 시 Copilot이 받는 컨텍스트:

┌─────────────────────────────────────────┐
│  copilot-instructions.md  (자동 로드)    │  ← 규칙
├─────────────────────────────────────────┤
│  #file:ARCHITECTURE.md    (수동 참조)    │  ← 설계
│  #file:context.md         (수동 참조)    │  ← 상태
│  #file:todo.md            (수동 참조)    │  ← 할 일
├─────────────────────────────────────────┤
│  열린 파일들               (자동 포함)    │  ← 현재 코드
└─────────────────────────────────────────┘
```

**세션 종료 프롬프트** — 작업을 마칠 때도 Copilot에게 맡긴다:
...
3. **설계서 = AI 컨텍스트**: ARCHITECTURE.md + copilot-instructions.md가 AI의 프로젝트 이해도를 높인다
4. **세션 간 연속성**: context.md(상태) + todo.md(할 일)로 Copilot의 기억 한계를 보완한다
