...
Next.js에서는 이 구조를 직접 작성하지 않는다. `app/layout.tsx`가 `<html>`과 `<body>`를 자동으로 감싸준다. 그러나 HTML의 기본 구조를 아는 것은 AI 출력을 검증할 때 필수이다.
...
> **라이브 코딩 시연**: 브라우저 DevTools에서 디바이스 모드(`Cmd+Shift+M`)를 켜고 화면 크기를 조절하며 반응형 변화를 보여준다.
...
<main className="max-w-4xl mx-auto p-4">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{/* 카드 반복 */}</div>
</main>
```
...
B회차에서는 Ch2에서 만든 블로그 프로젝트를 이어서 사용한다.
