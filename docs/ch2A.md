...
| 01:10~01:25 | 2.5~2.7: 검증 체크리스트 + Agent/Skills | 강의 (개념 설명)             |
...
> **Gemini CLI 사용**:
> Gemini CLI에서는 VS Code 메뉴 대신 터미널에서 바로 요청한다. 예: `gemini "Next.js App Router 프로젝트에 about 페이지 구조를 제안해줘"` → 결과를 읽고 파일을 직접 만들거나, 수정 지시를 이어서 입력한다.
...
> **Gemini CLI 사용**:
> Gemini CLI는 별도 VS Code 확장 설치 대신 터미널에서 실행한다. 프로젝트 폴더에서 `gemini` 명령이 동작하는지만 먼저 확인하고, 이후 프롬프트 기반으로 파일 생성·수정 요청을 진행하면 된다.
...
> **Gemini CLI 사용**:
> Gemini CLI에는 Copilot 아이콘이나 Chat 패널이 없다. 기준은 "현재 폴더에서 프롬프트를 받아 응답하는가"이다. 즉, VS Code 메뉴 확인 대신 터미널 세션이 프로젝트 루트를 가리키는지 확인하면 된다.
...
> **Gemini CLI 사용**:
> Gemini CLI는 인라인 자동완성보다 "짧은 지시 → 코드 제안" 흐름에 가깝다. 예: `gemini "현재 날짜를 2026년 2월 7일 형식으로 반환하는 JavaScript 함수를 작성해줘"`처럼 요청하고, 결과를 복사하거나 후속 수정 요청을 덧붙인다.
...
| `Ctrl + I` (macOS: `Cmd + I`) | 인라인 편집                   | 코드 위에서 직접 수정 지시                 |

> **Gemini CLI 사용**:
> Gemini CLI에는 `@workspace`, `/fix`, `/explain` 같은 UI 명령 대신 자연어로 직접 지시한다. 예: `gemini "이 프로젝트 전체 구조를 기준으로 /about 페이지를 추가하는 방법을 설명해줘"` 또는 `gemini "이 에러를 고쳐줘: ..."`처럼 요청한다.
...
Copilot Chat을 열고(Ctrl+Alt+I) 다음 프롬프트를 입력한다:
...
> - Known AI Mistakes: next/router 금지(next/navigation 사용), Pages Router 금지, @supabase/auth-helpers 금지(@supabase/ssr 사용)"

> **Gemini CLI 사용**:
> Gemini CLI에는 Copilot의 `copilot-instructions.md`처럼 자동 적용되는 전용 설정 파일이 분명하지 않을 수 있다. 대신 프로젝트 루트에 `GEMINI.md` 같은 규칙 문서를 두고, `gemini "GEMINI.md를 먼저 읽고 소개 페이지 작업 규칙을 반영해줘"`처럼 매 요청에서 참조시키는 방식이 현실적이다.
...
|  #  | 확인 항목                            | 예시                                           |
| :-: | ------------------------------------ | ---------------------------------------------- |
|  1  | package.json의 실제 버전이 정확한가? | Next.js 15.1.2 (15.x 같은 대략적 표기 금지)    |
|  2  | App Router ONLY 명시되어 있는가?     | "Pages Router 사용 금지" 문구                  |
|  3  | Known AI Mistakes 섹션이 있는가?     | next/router, getServerSideProps 같은 흔한 실수 |
...
> **Gemini CLI 사용**:
> Gemini CLI에는 `@workspace` 표기가 없으므로, 프롬프트에 "현재 프로젝트 전체 구조를 기준으로"라고 명시한다. 필요하면 `tree`, `dir`, `rg --files` 결과를 함께 붙여 컨텍스트를 보강한다.
...
> **Gemini CLI 사용**:
> Gemini CLI에서도 최신 문서 확인 원칙은 동일하다. 답변이 애매하면 "공식 문서 기준으로 확인해줘"를 함께 적고, 필요한 경우 공식 사이트를 직접 열어 교차 검증한다.
...
3. **copilot-instructions.md의 "Known AI Mistakes"에 추가**한다
...
**⑤번이 가장 중요하다.** AI는 기억하지 못하지만, copilot-instructions.md는 기억한다.
...
> **Gemini CLI 사용**:
> Gemini CLI에는 Copilot의 Ask/Edit/Agent 같은 고정 메뉴가 없더라도, 긴 작업을 한 번에 맡길 수 있다. 다만 "어떤 파일을 수정할지", "무엇은 건드리지 말지", "먼저 계획을 말한 뒤 수정할지"를 프롬프트에 더 명확히 써야 한다.
...
**표 2.10** copilot-instructions.md vs Agent Skills
...
> **Gemini CLI 사용**:
> Gemini CLI에는 Copilot Skills와 완전히 같은 자동 발동 장치는 없을 수 있다. 대신 `GEMINI.md`, `AGENTS.md`, 프로젝트 규칙 문서 같은 고정 컨텍스트 파일을 두고, 매 요청에서 "프로젝트 규칙 문서를 먼저 따르라"라고 명시하는 방식이 실용적이다.

### 2.7.3 참고 — MCP: 외부 도구 연결 (선택)
...
**등록 방법**: `Ctrl+Shift+P` (macOS: `Cmd+Shift+P`) → `MCP: Add Server` → 목록에서 선택

> **quota 주의**: 학생 Pro 계정은 월 300회 premium request 제한. MCP 도구 호출은 quota를 소모하므로 처음에는 **Skills만 사용**하는 것을 권장한다. 자세한 설정 방법은 부록 C 참고.

> **Gemini CLI 사용**:
> Gemini CLI 사용자도 "최신 문서 연결"과 "외부 도구 연결"을 구분해서 생각하면 된다. 모든 기능을 처음부터 붙이기보다, 기본 프롬프트 흐름이 안정된 뒤 문서 검색이나 배포 도구 연동을 추가하는 편이 낫다.
