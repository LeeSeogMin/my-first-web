...
2. 그 폴더 안에 **`page.tsx`**를 만든다 = 해당 경로의 페이지가 된다
3. `page.tsx`가 없는 폴더는 URL을 생성하지 않는다

> **라이브 코딩 시연**: 프로젝트에 `app/about/page.tsx`를 만들고 브라우저에서 `/about`에 접속하는 과정을 보여준다. "폴더를 만들면 페이지가 된다"를 직접 확인시킨다.

### 5.2.2 page.tsx — 페이지 정의

`page.tsx`는 해당 URL에서 보여줄 내용을 정의하는 파일이다. React 컴포넌트를 **`export default`**로 내보내면 된다:
...
> **실습 안내**: `app/posts/page.tsx`를 만들고 브라우저에서 `/posts` 접속 결과를 확인한다.

**코드 읽기 포인트**:
- `export default function` — 페이지 컴포넌트는 반드시 **default export**
- 함수 이름은 관례적으로 `[경로명]Page` (예: PostsPage, AboutPage)
- 하나의 `page.tsx`에 하나의 default export 컴포넌트

### 5.2.3 layout.tsx — 공통 레이아웃

`layout.tsx`는 여러 페이지에 **공통으로 적용되는 구조**(내비게이션 바, 푸터 등)를 정의한다. `app/layout.tsx`는 모든 페이지에 적용되는 **루트 레이아웃**(Root Layout)이다:
...
### 5.2.4 loading.tsx, error.tsx — 특수 파일

**표 5.4** Next.js 특수 파일 요약

| 파일 | 역할 | 적용 범위 |
|------|------|-----------|
| `page.tsx` | 해당 URL의 페이지 내용 | 해당 경로만 |
| `layout.tsx` | 공통 구조 (네비, 푸터) | 해당 경로 + 하위 모든 경로 |
| `loading.tsx` | 로딩 중 표시할 UI | 해당 경로 |
| `error.tsx` | 에러 발생 시 표시할 UI | 해당 경로 |
| `not-found.tsx` | 404 페이지 | 해당 경로 |

### 5.2.4 loading.tsx — 로딩 UI


`loading.tsx`를 만들면 해당 경로의 페이지가 로딩 중일 때 자동으로 표시된다:
...
`error.tsx`는 해당 경로에서 에러가 발생했을 때 보여줄 UI를 정의한다:
...
블로그에서 각 게시글은 고유한 URL을 가진다: `/posts/1`, `/posts/2`, `/posts/3`... 매번 `app/posts/1/page.tsx`, `app/posts/2/page.tsx`를 만들 수는 없다. **동적 라우트**(Dynamic Route)가 이 문제를 해결한다.
...
export default async function PostDetailPage({
  ...
  const { id } = await params; // Next.js 16(App Router) 기준
...
> ⚠️ **이 교재 고정 버전(Next.js 16.2.1) 주의사항**: `params`는 Promise로 전달된다. 따라서 `const { id } = await params`처럼 비동기로 꺼내야 한다. AI가 예전 버전 문법으로 객체처럼 바로 접근하면 현재 실습 기준과 맞지 않는다.

**코드 읽기 포인트**:
- `const { id } = await params` — Ch4에서 배운 구조 분해 할당을 비동기 props에 적용
...
function PostItem({ id, title, date }: PostItemProps) {
  return (
    <Link
      href={`/posts/${id}`}
...
> **실습 안내**: `app/layout.tsx`의 내비게이션 바에 NavLink 컴포넌트를 적용하고, 페이지 이동 시 활성 링크 변화를 확인한다. NavLink는 `components/NavLink.tsx`로 분리한다.
